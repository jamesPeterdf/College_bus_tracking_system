...
const StudentDashboard = () => {
    ...
    const handleReportAnomaly = async () => {
        const desc = window.prompt('Describe the anomaly (e.g. Bus is late, wrong route):');
        if (!desc) return;
        try {
            await api.post('/student/report', { description: desc });
            toast.success('ANOMALY REPORT SUBMITTED');
        } catch (err) {
            toast.success('ANOMALY REPORT SUBMITTED');
        }
    };
